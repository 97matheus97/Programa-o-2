...
void counting_sort(dado_t **dados, int n_linhas)
{
    ...
    int j;
    int max = max;
    dado_t **temp = malloc(sizeof(struct dados) * max);
...
void counting_sort(dado_t **dados, int n_linhas)
{
    ...
    int max = max;
    dado_t **temp = malloc(sizeof(struct dados*) * max+1);
    dado_t **temp2 = malloc(sizeof(struct dados*) * n_linhas);
    ...
    for(j=n_linhas-1; j>0;n_linhas--){
        temp2[temp[dados[i]->altura]] = dados[j]->altura;
        temp[dados[j]->altura] = temp[dados[j]->altura] - 1;
    }
